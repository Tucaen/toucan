...
- ADE hosts the FirstMate captain as an ACP conversation, not a tmux pane. Request/task pinning is owned by `src/shared/firstmate-task-context.ts` and `src/renderer/src/firstmate-request-target.ts`; lifecycle, app-native wake, serialized task-scoped validation, and provider/model wrappers are owned by `src/main/firstmate-lifecycle.ts`, `src/main/firstmate-lifecycle-coordinator.ts`, and `src/main/firstmate-runtime.ts`.
