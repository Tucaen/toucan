...
Sidebar projects stay where they are. The first request for a project registers it with FirstMate as a durable external project: ADE records its stable project identity, canonical Windows and WSL paths, Git origin when it has one, delivery posture, and autonomy posture in its own registration file inside the private FirstMate home, and restores that mapping on restart. A project with a remote defaults to FirstMate's `no-mistakes-prod-only` standing posture, a project without one defaults to `local-only`, and autonomy stays off until the captain asks for it. The dock shows the registered posture beside the request target. Two projects with the same folder name stay distinct, and ADE never clones or links a checkout into FirstMate's managed `projects` directory.
