...
Feature plans that spawned a ticket set live under `docs/plans/`, and each issue links its plan.

## Commit messages carry the issue number

Subjects are `type(affected-feature): ticket-number commit-message` - e.g. `feat(agent): 127 fan agent events out through a main-process broker` - the bare number, no `#`. Full rules in AGENTS.md.
