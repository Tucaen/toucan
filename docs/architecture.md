...
src/shared holds runtime-neutral contracts and domain rules, importable by any side.
...
Delegation cards read provider model/token evidence through
[`src/shared/delegation-evidence.ts`](../src/shared/delegation-evidence.ts), folded by
`mergeActivity` into the shared transcript. Built-in/MCP tool identity is shared in
[`tool-identity.ts`](../src/shared/tool-identity.ts) so evidence and cards classify the same
call. Missing telemetry stays unavailable; overlapping worker totals are never added.
See [transport limits and measured comparison](plans/delegation-evidence.md).

| Area                             | Owner                                                                                                                                                                                                                  | Responsibility and interface                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Persisted domain contracts       | [`src/shared/workspace.ts`](../src/shared/workspace.ts), [`terminal.ts`](../src/shared/terminal.ts), [`agent.ts`](../src/shared/agent.ts), [`worktree.ts`](../src/shared/worktree.ts), [`conversation.ts`](../src/shared/conversation.ts)                            | Cross-process data shapes, validation-independent domain rules, and stable identities.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| Cross-cutting pure rules         | [`src/shared`](../src/shared)                                                                                                                                                                                          | Attention, activity folding, worktree handoff, titles, stall guards, and text normalization. These modules know no runtime process. Membership is that neutrality, not a headcount of importers: a rule only main runs today (`worktree-identity.ts`, `session-outcome.ts`) or only the renderer does (`worktree-handoff.ts`) belongs here as long as it names no runtime, because that is what keeps it testable in isolation and free to be reached from the other side or the phone without moving.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| Terminal shell                   | [`src/main/terminal-shell.ts`](../src/main/terminal-shell.ts)                                                                                                                                                    | Which shell a plain terminal launches: PowerShell 7, Windows PowerShell, then the environment's own `ComSpec`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| Account usage                    | [`src/main/provider-usage.ts`](../src/main/provider-usage.ts), [`src/renderer/src/ProviderUsageChip.tsx`](../src/renderer/src/ProviderUsageChip.tsx), [`src/renderer/src/use-provider-rate-limits.ts`](../src/renderer/src/use-provider-rate-limits.ts), [`mobile/src/use-provider-usage.ts`](../mobile/src/use-provider-usage.ts)                                                                                                                                                          | Provider-neutral caching interface over the Claude and Codex usage adapters. A reading kept through a failed read is reported as stale rather than passing for a fresh one, and the header chip dates and confirms its own reading - plan usage barely moves between two clicks, so without that a refresh that worked looks like a button that does nothing. Each provider refreshes on its own, so a slow or missing CLI cannot disable the other provider's chip. The display maths is [`src/shared/session-usage.ts`](../src/shared/session-usage.ts) rather than the renderer's, because the phone shows the same windows over `GET /api/usage` ([`src/shared/remote-usage.ts`](../src/shared/remote-usage.ts)) and two surfaces deriving one threshold separately is how a window comes to read as `warning` on one device and `critical` on the other. That route is read-only and cache-served: a forced read boots a provider CLI process, which a paired client does not get to trigger.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| Mobile companion client          | [`mobile/`](../mobile)                                                                                                                                                                                                 | A routed, multi-host client as a static build served at `/`, addressed by pathname so the browser's back button and a reloaded deep link both land somewhere sensible ([`routes.ts`](../mobile/src/routes.ts)). The saved hosts it is parameterized by live in [`hosts.ts`](../mobile/src/hosts.ts), their reachability and probe schedule in [`host-status.ts`](../mobile/src/host-status.ts), and the persistence and timers wiring both into the screens in [`use-host-directory.ts`](../mobile/src/use-host-directory.ts). Its list decisions live in [`chat-list.ts`](../mobile/src/chat-list.ts), its connection, composer and answer state in [`chat-connection.ts`](../mobile/src/chat-connection.ts), its question-set form rules in [`decision-answers.ts`](../mobile/src/decision-answers.ts), what the "New chat" form may assume in [`new-chat.ts`](../mobile/src/new-chat.ts), its account-usage cards in [`provider-usage.ts`](../mobile/src/provider-usage.ts) behind the single host poll in [`use-provider-usage.ts`](../mobile/src/use-provider-usage.ts), and its transport in [`remote-client.ts`](../mobile/src/remote-client.ts). Whether the page may install itself is [`install-support.ts`](../mobile/src/install-support.ts), and what its service worker ([`sw.ts`](../mobile/src/sw.ts)) may cache is [`service-worker-policy.ts`](../mobile/src/service-worker-policy.ts); the manifest and icons are static files in [`mobile/public/`](../mobile/public). |
| Voice input                      | [`src/renderer/src/VoiceInput.tsx`](../src/renderer/src/VoiceInput.tsx), [`mobile/src/MobileVoiceInput.tsx`](../mobile/src/MobileVoiceInput.tsx), [`src/main/voice-transcription.ts`](../src/main/voice-transcription.ts) | Dictation on both surfaces, decoded in one place: main runs whisper.cpp large-v3-turbo as a hidden `whisper-server` child ([`whisper-engine.ts`](../src/main/whisper-engine.ts)) behind a lazy-load, serialize, idle-release policy. The desktop composer records PCM and hands the whole utterance over on Stop, biased by [`voice-transcript.ts`](../src/renderer/src/voice-transcript.ts)'s context passage as the initial prompt; the phone prefers its browser's own recognizer and otherwise posts the same raw PCM to `/api/transcribe`. Pins and asset ownership are [`src/shared/whisper-assets.ts`](../src/shared/whisper-assets.ts) and [`src/main/voice-model-store.ts`](../src/main/voice-model-store.ts); the wire contract is [`src/shared/remote-voice.ts`](../src/shared/remote-voice.ts); the phone's decisions are [`mobile/src/voice-input.ts`](../mobile/src/voice-input.ts). See [voice-input.md](voice-input.md). |
| Dictation cleanup | [`src/main/dictation-cleanup.ts`](../src/main/dictation-cleanup.ts), [`src/main/claude-dictation-cleanup.ts`](../src/main/claude-dictation-cleanup.ts), [`src/renderer/src/use-dictation-cleanup.ts`](../src/renderer/src/use-dictation-cleanup.ts) | Optional desktop transcript polishing after local transcription. The shared [`dictation-cleanup.ts`](../src/shared/dictation-cleanup.ts) defines the workspace preference, fixed model choices and IPC contract. Main owns the deadline, defensive parsing and raw fallback; the Claude runner owns the hidden, tool-free, non-persistent process. [`dictation-cleanup-ipc.ts`](../src/main/dictation-cleanup-ipc.ts) scopes cancellation to the invoking window. The renderer retains the original text for immediate cancellation and transport failure. No ACP session, broker or conversation persistence participates. |
...
| Canvas orchestration             | [`src/renderer/src/App.tsx`](../src/renderer/src/App.tsx), [`canvas-workspace.ts`](../src/renderer/src/canvas-workspace.ts), [`node-snap.ts`](../src/renderer/src/node-snap.ts)                                          | Compose workspace modules, projects, worktrees, React Flow nodes, recently closed sessions, conversion between persisted and live nodes, and session-local snap/restore geometry.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| Agent conversation orchestration | [`src/renderer/src/use-agent-conversation.ts`](../src/renderer/src/use-agent-conversation.ts), [`ChatNode.tsx`](../src/renderer/src/ChatNode.tsx)                                                                      | Wire conversation state to the rendered chat. Event-to-transcript folding is the shared reducer in [`src/shared/agent-transcript.ts`](../src/shared/agent-transcript.ts); the hook owns only renderer-local delivery state (attachments, outbox, optimistic sends) and IPC. Transcript decisions are delegated to pure feature modules; the composer's text belongs to the node and reaches the hook as `submit`'s prompt argument, via the prompt editor below.                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Renderer feature logic           | Small non-TSX modules in [`src/renderer/src`](../src/renderer/src)                                                                                                                                                     | Pure decisions for composer behavior, queued prompts, decisions, activity/card recognition, scrolling, liveness presentation, and worktree removal. Usage readouts used to live here and now sit in [`src/shared/session-usage.ts`](../src/shared/session-usage.ts), because the phone renders them too. Tests call these interfaces directly; TSX modules render their results.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| Ticket files                     | [`src/main/ticket-library.ts`](../src/main/ticket-library.ts), [`ticket-watcher.ts`](../src/main/ticket-watcher.ts), [`ticket-directory.ts`](../src/main/ticket-directory.ts)                                          | The files ticket source: listing, diagnostics and atomic status rewrites over a project's Markdown tickets, plus one debounced watcher per open project and the rule for which folder a project uses (never one outside the checkout). What a conforming ticket is lives in [`src/shared/tickets.ts`](../src/shared/tickets.ts); nothing here caches a ticket, because the files are the truth. That same listing is also what enforces the convention on the *writing* side: [`ticket-steering.ts`](../src/main/ticket-steering.ts) re-runs it on every watcher tick and tells whichever session wrote a file the board cannot read, with the decision (who to tell, once) in [`src/shared/ticket-conformance.ts`](../src/shared/ticket-conformance.ts) and the authorship evidence in `AcpSessionManager.recentWrites()`.                                                                                                                                                                                                                                                                                                                                                      |
...
| `src/main/remote/**`          | `src/shared/**`, other `src/main` modules, and Node's HTTP, crypto and fs                                   | Electron's `app`/`BrowserWindow`/`ipcMain` beyond the composition root's wiring, and any renderer module               |
...
- **Preload interfaces** are the privilege seam. Each `window.*Api` contract lives in
  ...
  transport change should be possible behind `window.workspaceApi`, `window.terminalApi`,
  `window.shellApi`, `window.agentApi`, `window.usageApi`, `window.worktreeApi`, and
  `window.conversationApi` without changing canvas behavior.
...
- voice input, contract then policy then screens: [`remote-voice.test.ts`](../tests/remote-voice.test.ts), [`voice-transcription.test.ts`](../tests/voice-transcription.test.ts), [`whisper-engine.test.ts`](../tests/whisper-engine.test.ts), [`voice-transcript.test.ts`](../tests/voice-transcript.test.ts), [`mobile-voice-input.test.ts`](../tests/mobile-voice-input.test.ts), [`mobile-voice-input.dom.test.tsx`](../tests/mobile-voice-input.dom.test.tsx)
