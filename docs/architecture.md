...
| Account usage                    | [`src/main/provider-usage.ts`](../src/main/provider-usage.ts), [`src/renderer/src/ProviderUsageChip.tsx`](../src/renderer/src/ProviderUsageChip.tsx), [`src/renderer/src/use-provider-rate-limits.ts`](../src/renderer/src/use-provider-rate-limits.ts)                                                                                                                                                          | Provider-neutral caching interface over the Claude and Codex usage adapters. A reading kept through a failed read is reported as stale rather than passing for a fresh one, and the header chip dates and confirms its own reading - plan usage barely moves between two clicks, so without that a refresh that worked looks like a button that does nothing. Each provider refreshes on its own, so a slow or missing CLI cannot disable the other provider's chip.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
