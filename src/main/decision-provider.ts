 ...
 *
 * It reads the plugin registry Claude Code maintains, `~/.claude/plugins/installed_plugins.json`,
 * and asks only whether the constant plugin id has an install recorded. The install path beside
 * each entry carries a version, so globbing the cache directory would break on every bump; the id
 * does not. Presence of the skill is the whole question - the API key is the skill's own business
 * and a missing one fails visibly in the transcript, where the user can see it.
 *
 ...
 */
export function isDecisionProviderInstalled(homeDirectory: string, readFile = readFileSync): boolean {
  try {
    const registry = JSON.parse(
      readFile(join(homeDirectory, '.claude', 'plugins', 'installed_plugins.json'), 'utf8') as string
