...
export function isTerminalCreateRequest(value: unknown): value is TerminalCreateRequest {
  return isRecord(value) && isString(value.id) && value.kind === 'terminal' &&
    isString(value.cwd) && isTerminalSize(value.cols, value.rows) &&
    optionalString(value.sessionId) && optionalString(value.attachmentId) &&
