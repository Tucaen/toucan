...
function isDiffFile(value: unknown): value is GitChangedFile {
  return isRecord(value) && isString(value.path) && optionalString(value.oldPath) &&
    ['added', 'modified', 'deleted', 'renamed', 'copied', 'type-changed', 'unmerged', 'untracked'].includes(String(value.status)) &&
    [value.added, value.deleted].every((count) => count === undefined ||
...
export function registerWorktreeIpc(
  ...
  const allowed = async (path: unknown): Promise<boolean> =>
    isString(path) && await containment.contains(path)
  ipc.handle(WORKTREE_CHANNELS.create, async (_event, request) => {
    if (!isRecord(request) || !isString(request.projectPath) || !isString(request.branch) ||
      !optionalString(request.baseRef) || !(await allowed(request.projectPath))) return REFUSED
  ...
  ipc.handle(WORKTREE_CHANNELS.status, async (_event, request) => {
    if (!isRecord(request) || !isString(request.path) || !isString(request.branch) ||
      !isString(request.baseRef) || !(await allowed(request.path)))
      return { exists: true, changedFiles: 0, untrackedFiles: 0, ahead: 0, behind: 0,
        hasUpstream: false, stashEntries: 0, message: MESSAGE }
  ...
  ipc.handle(WORKTREE_CHANNELS.remove, async (_event, request) => {
    if (!isRecord(request) || !isString(request.projectPath) || !isString(request.path) ||
      !isString(request.branch) || !isString(request.baseRef) ||
      (request.force !== undefined && typeof request.force !== 'boolean') ||
      !(await allowed(request.projectPath)) || !(await allowed(request.path)))
      return { ...REFUSED, blockers: [{ kind: 'inspection-failed', detail: MESSAGE }] }
    return worktrees.remove({ projectPath: request.projectPath, path: request.path,
      branch: request.branch, baseRef: request.baseRef, force: request.force })
  })
  ipc.handle(WORKTREE_CHANNELS.discover, async (_event, request) => {
    const refused = { worktrees: [], claims: [], message: MESSAGE }
    if (!isRecord(request) || !isString(request.projectPath) || !Array.isArray(request.known) ||
      !request.known.every(isString) || !(await allowed(request.projectPath))) return refused
  ...
  ipc.handle(WORKTREE_CHANNELS.diff, async (_event, request) => {
    if (!isRecord(request) || !isString(request.path) || !isString(request.baseRef) ||
      !(await allowed(request.path))) return { ...REFUSED, reason: 'failed' }
  ...
  ipc.handle(WORKTREE_CHANNELS.diffFile, async (_event, request) => {
    if (!isRecord(request) || !isString(request.path) || !isString(request.baseRef) ||
      !isDiffFile(request.file) || !(await allowed(request.path))) return REFUSED
  ...
  ipc.handle(WORKTREE_CHANNELS.checkoutBranch, async (_event, request) => {
    if (!isRecord(request) || !isString(request.path) || !isString(request.branch) ||
      !(await allowed(request.path))) return REFUSED
