...
export function initialAssistantPresentation(
  presentation: AgentMessagePresentation | undefined
): { presentation: AgentMessagePresentation; presentationProvisional: boolean } {
  return presentation
    ? { presentation, presentationProvisional: false }
    : { presentation: 'progress', presentationProvisional: true }
}

function settleAssistantGroup<T extends AssistantPresentationMessage>(
  messages: T[],
  indices: readonly number[]
): void {
  const provisional = indices.filter((index) => messages[index].presentationProvisional === true)
  const hasProviderFinal = indices.some((index) => (
    messages[index].presentation === 'final' && messages[index].presentationProvisional !== true
  ))
