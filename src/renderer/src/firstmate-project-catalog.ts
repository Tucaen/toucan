...
function dispatchContract(): string {
  return [
    ...
    '- before calling `fm-spawn.sh`, validate that selected entry\'s exact taskContextMetadata through the managed ADE spawn gate: `$FM_HOME/bin/ade-spawn-gate <taskContextMetadata>`; if the gate exits non-zero, launch no worker and report its stderr to the captain; the gate checks the pinned context against ADE\'s authoritative project record and current validator;',
    '- only after the gate passes, use managed `fm-spawn.sh`, whose Git guard proves the allocated directory is a disposable worktree rather than the primary checkout;',
