...
function shellIdFrom(input: Record<string, unknown>): string | undefined {
  return (
    asText(input.bash_id) ?? asText(input.bashId) ?? asText(input.shell_id) ?? asText(input.shellId) ?? asText(input.id)
  )
}
...
export function parseShellExecution(activity: AgentActivity): ShellExecution | null {
  ...
  const cwd = activity.terminalCwd ?? asText(input.cwd) ?? asText(input.workdir) ?? asText(input.working_dir)
  ...
  return {
    ...
    ...(cwd ? { cwd } : {}),
    ...(shellId ? { shellId } : {}),
    ...(asText(input.description) ? { description: asText(input.description) } : {}),
    ...exit
  }
}
...
export function shellCommandLine(command: string): string {
  ...
  return flattened.length > COMMAND_SUMMARY_LIMIT ? `${flattened.slice(0, COMMAND_SUMMARY_LIMIT - 1)}…` : flattened
}
...
export function shellExitTone(execution: ShellExecution, status: AgentActivity['status']): 'ok' | 'error' | undefined {
  if (execution.exitSignal) return 'error'
  if (execution.exitCode !== undefined) return execution.exitCode === 0 ? 'ok' : 'error'
...
export function shellOutputBlocks(activity: AgentActivity): ShellOutputBlock[] {
  const rawOutput = asRecord(activity.rawOutput)
  const stdout = typeof rawOutput?.stdout === 'string' ? rawOutput.stdout : undefined
  const stderr = typeof rawOutput?.stderr === 'string' ? rawOutput.stderr : undefined
 ...
 */
export function indexShellLaunches(activities: readonly AgentActivity[]): Map<string, ShellLaunch> {
  const launches = new Map<string, ShellLaunch>()
  for (const activity of activities) {
    const execution = shellExecutionFor(activity)
    if (execution?.kind !== 'run' || !execution.shellId || !execution.command) continue
