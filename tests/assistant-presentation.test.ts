...
test('unphased assistant text is provisional progress until its turn completes', () => {
  ...
  assert.deepEqual(settled.map(({ presentation, presentationProvisional, complete }) => ({
    presentation, presentationProvisional, complete
  })), [
...
test('replay settles each user turn independently', () => {
  ...
  assert.deepEqual(settled.filter((message) => message.role === 'assistant').map((message) => message.presentation), [
    'progress',
    'final',
    'final'
  ])
