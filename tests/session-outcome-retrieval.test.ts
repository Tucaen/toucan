...
test('the pointer names the directory, every field a record carries, and how to filter by project', () => {
  ...
  // The worked example in the pointer is generated by the same function the tests exercise, so the
  // prose recipe and the pattern the suite proves against the serializer cannot drift apart.
  assert.ok(instruction.includes(sessionOutcomeProjectPattern('D:\\Dev\\App')))
...
test('the taught grep pattern survives Bash on Windows and selects only its own project (#197)', () => {
  // The failure this locks out was silent: Bash on Windows mangles backslash runs in arguments
  // before grep sees them, so the literal JSON-quoted path the pointer used to teach matched
  // nothing and read as "this project has no records". The taught pattern therefore may not
  // contain a single backslash - dots stand in for them - and it is proven here against a record
  // written by the real serializer, not a hand-built fixture.
  const pattern = sessionOutcomeProjectPattern('D:\\Development\\ADE')
  assert.ok(!pattern.includes('\\'), `the taught pattern still carries a backslash: ${pattern}`)

  const rendered = (projectPath: string): string => {
    const record = extractSessionOutcome(
      transcript(user('u1', 'Try the taught retrieval pattern.'), assistant('a1', 'Tried it.'), {
        type: 'turn_complete',
        stopReason: 'end_turn'
      }),
    ...
    assert.ok(record)
    return renderSessionOutcome(record)
  }
  const regex = new RegExp(pattern)
  assert.match(rendered('D:\\Development\\ADE'), regex)
  // Selectivity: the closing quote and the full-path shape keep out a same-named leaf under a
  // different root, a sibling sharing the prefix, and this project's own worktrees.
  assert.doesNotMatch(rendered('C:\\Other\\ADE'), regex)
  assert.doesNotMatch(rendered('D:\\Development\\ADE-legacy'), regex)
  assert.doesNotMatch(rendered('D:\\Development\\ADE\\worktrees\\fix'), regex)
  // A path with no backslashes passes through unchanged: the one recipe works on every platform.
  const posix = sessionOutcomeProjectPattern('/home/ada/app')
  assert.equal(posix, 'project: "/home/ada/app"')
  assert.match(rendered('/home/ada/app'), new RegExp(posix))
})
