...
test('the pointer names the directory, every field a record carries, and how to filter by project', () => {
  ...
  const record = extractSessionOutcome(
    ...
    ),
    {
      ...
      worktreeId: 'wt-1',
      commit: '93ff65b0c2f1d4e5a6b7c8d9e0f1a2b3c4d5e6f7',
      branch: 'main'
    },
    null,
    AT
  ...
  // Issue #17: a recorded failure is only as current as the code it was observed against, so the
  // pointer teaches the freshness check - and that a commit git no longer knows is not "unchanged".
